...
Функция ПолучитьДанныеПоЗаказамOZON(Ресурс, ТелоЗапроса, ВидОперации = "")
	...
	Если ПустаяСтрока(ТелоЗапроса) Тогда
		...
		ИначеЕсли ВидОперации = "ВыполнитьСборкуOZON" Тогда
			Сообщить("Не найден номер отправления по текущему заказу. Либо статус заказа отличный от статуса ""Ожидает сборки""");
		ИначеЕсли ВидОперации = "ОткрытьСпорПоОтправлениюСOZON" Тогда
			Сообщить("Не найден номер отправления по текущему заказу. Либо статус заказа отличный от статуса ""Не принят на пункте сортировки""");
		КонецЕсли;
		Возврат Данные;
	КонецЕсли;
